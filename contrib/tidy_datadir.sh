...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Cryptotarget100M database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Cryptotarget100M datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Cryptotarget100M datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Cryptotarget100M 0.7 datadir."
    ;;
  3)
    echo "Detected Cryptotarget100M pre-0.8 datadir."
    ;;
  4)
    echo "Detected Cryptotarget100M 0.8 datadir."
    ;;
esac
