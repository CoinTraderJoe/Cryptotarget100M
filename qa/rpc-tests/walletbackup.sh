...
CRYPTOTARGET100MD=${1}/cryptotarget100Md
CLI=${1}/cryptotarget100M-cli
...
B4ARGS="-datadir=$D4"
$CRYPTOTARGET100MD $CRYPTOTARGET100MDARGS $B4ARGS &
B4PID=$!
...
function CreateConfDir {
  DIR=$1
  mkdir -p $DIR
  CONF=$DIR/cryptotarget100M.conf
  echo "regtest=1" >> $CONF
...
# 1 block, 50 XBT each == 50 CRT100M
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} crt100m"
...
function StartThree {
  $CRYPTOTARGET100MD $CRYPTOTARGET100MDARGS $B1ARGS &
  B1PID=$!
  $CRYPTOTARGET100MD $CRYPTOTARGET100MDARGS $B2ARGS &
  B2PID=$!
  $CRYPTOTARGET100MD $CRYPTOTARGET100MDARGS $B3ARGS &
  B3PID=$!
}
