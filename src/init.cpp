...
void Shutdown()
{
    ...
    RenameThread("cryptotarget100M-shutoff");
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateCryptotarget100Ms(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: cryptotarget100M.conf)") + "\n";
    if (hmm == HMM_CRYPTOTARGET100MD)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: cryptotarget100Md.pid)") + "\n";
    ...
    if (hmm == HMM_CRYPTOTARGET100M_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Cryptotarget100M Wiki for SSL setup instructions)") + "\n";
...
{
    RenameThread("cryptotarget100M-loadblk");
...
/** Sanity checks
 *  Ensure that Cryptotarget100M is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.cryptotarget100M.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Cryptotarget100M Core is shutting down."));
...
#endif
    // Make sure only a single Cryptotarget100M process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Cryptotarget100M Core is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Cryptotarget100M version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Cryptotarget100M") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Cryptotarget100M to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateCryptotarget100Ms(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
