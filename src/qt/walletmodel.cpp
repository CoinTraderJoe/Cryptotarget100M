...
{
    CCryptotarget100MAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered cryptotarget100M address / amount:
            ...
            scriptPubKey.SetDestination(CCryptotarget100MAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal cryptotarget100M:URI (cryptotarget100M:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CCryptotarget100MAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CCryptotarget100MAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    CTxDestination dest = CCryptotarget100MAddress(sAddress).Get();
