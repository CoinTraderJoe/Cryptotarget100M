...
const int CRYPTOTARGET100M_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString CRYPTOTARGET100M_IPC_PREFIX("cryptotarget100M:");
const char* CRYPTOTARGET100M_REQUEST_MIMETYPE = "application/cryptotarget100M-paymentrequest";
const char* CRYPTOTARGET100M_PAYMENTACK_MIMETYPE = "application/cryptotarget100M-paymentack";
const char* CRYPTOTARGET100M_PAYMENTACK_CONTENTTYPE = "application/cryptotarget100M-payment";
...
{
    QString name("Cryptotarget100MQt");
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(CRYPTOTARGET100M_IPC_PREFIX, Qt::CaseInsensitive)) // cryptotarget100M: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseCryptotarget100MURI(arg, &r) && !r.address.isEmpty())
            {
                CCryptotarget100MAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(CRYPTOTARGET100M_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(CRYPTOTARGET100M_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click cryptotarget100M: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start cryptotarget100M: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling cryptotarget100M: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on cryptotarget100M: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in cryptotarget100M: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(CRYPTOTARGET100M_IPC_PREFIX, Qt::CaseInsensitive)) // cryptotarget100M: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseCryptotarget100MURI(s, &recipient))
                emit receivedPaymentRequest(recipient);
            else
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Cryptotarget100M address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CCryptotarget100MAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()){
            // Insecure payments to custom cryptotarget100M addresses are not supported
        ...
        if (txOut.IsDust(CTransaction::nMinRelayTxFee)) {
            QString msg = tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(Cryptotarget100MUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second));
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", CRYPTOTARGET100M_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, CRYPTOTARGET100M_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", CRYPTOTARGET100M_PAYMENTACK_MIMETYPE);
